#same as formula loop but for elasticities, basically only difference is we have different variables





loopformulaELAS <- function(data,columns, folderpath, idealvector,start,chunks) {
  ...
  out <- unlist(lapply(1:length(X), function(n) combn(X, n, FUN=function(row) paste0("T_Elast ~ Elas_Precision  + (1 | Study_ID) + ", paste0(row, collapse = "+")))))
  
  #add in FAT-PET with no covariates
  out <- c("T_Elast ~ Elas_Precision  + (1 | Study_ID)", out)
  ...
  for (N in start:length(n)) {
    ...
    for (i in 1:p) {
      ...
      for (j in 1:length(X)) {
        ...
        #These lines find the coefficent order in the formula (if it is in this specification) 
        #And then put coefficient into dataframe
        ...
        #this part checks if coefficent standard error exists in each specification
  ...
  #=================================================
  #This gets the sample average and then combines them with coeffients to get estimates of average effect size
